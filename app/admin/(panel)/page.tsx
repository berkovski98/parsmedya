...
import { AnalyticsError, MetricCards, TopPages } from '@/components/admin/analytics-widgets'
...
export default async function DashboardPage() {
  ...
  const [{ data }, analyticsResult, contactsResult, newContacts, todayContacts, weekContacts] = await Promise.all([
  ...
  return <div className="mx-auto max-w-7xl"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="font-display text-3xl font-bold">Dashboard</h1><p className="mt-2 text-muted-foreground">Gerçek ziyaret, iletişim ve blog yayın verilerinizi takip edin.</p></div><Link href="/admin/blog/new" className="w-fit rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground">Yeni Yazı</Link></div>{!analyticsResult.ok && <div className="mt-8"><AnalyticsError error={analyticsResult.error} /></div>}{trackingDisabled && analyticsResult.ok && <p className="mt-8 rounded-lg border border-border bg-secondary/60 p-4 text-sm text-muted-foreground">Ziyaretçi takibi kapalı: {trackingDisabled}. Production için NEXT_PUBLIC_ANALYTICS_ENABLED=true ve ANALYTICS_ENABLED=true ayarlayın.</p>}<section className="mt-8"><h2 className="sr-only">Yönetim istatistikleri</h2><MetricCards items={metrics} /></section><div className="mt-8 grid gap-6 lg:grid-cols-2"><section className="rounded-xl border border-border bg-card p-6"><div className="flex items-center justify-between"><h2 className="font-display text-xl font-bold">En Çok Ziyaret Edilen Sayfalar</h2><Link href="/admin/analytics" className="text-sm font-medium text-accent hover:underline">Tüm istatistikler</Link></div><div className="mt-6">{analytics ? <TopPages pages={analytics.top_pages.slice(0, 7)} /> : <p className="text-sm text-muted-foreground">Sayfa istatistikleri yüklenemedi.</p>}</div></section><section className="rounded-xl border border-border bg-card p-6"><div className="flex items-center justify-between"><h2 className="font-display text-xl font-bold">Son İletişim Talepleri</h2><Link href="/admin/contact" className="text-sm font-medium text-accent hover:underline">Tüm talepler</Link></div><div className="mt-4 divide-y divide-border">{recentContacts.map((contact) => <Link key={contact.id} href={`/admin/contact/${contact.id}`} className="block py-3"><div className="flex items-center justify-between gap-3"><p className="truncate text-sm font-medium">{contact.name}</p><span className="text-xs text-muted-foreground">{new Date(contact.created_at).toLocaleDateString('tr-TR')}</span></div><p className="mt-1 truncate text-xs text-muted-foreground">{contact.subject || contact.email}</p></Link>)}{recentContacts.length === 0 && <p className="py-4 text-sm text-muted-foreground">Henüz iletişim talebi yok.</p>}</div></section><section className="rounded-xl border border-border bg-card p-6 lg:col-span-2"><h2 className="font-display text-xl font-bold">Blog Durumu</h2><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{blogMetrics.map((item) => <div key={item.label} className="rounded-lg bg-secondary/60 p-4"><p className="text-xs text-muted-foreground">{item.label}</p><p className="mt-1 font-display text-2xl font-bold">{item.value}</p></div>)}</div><h3 className="mt-6 font-display font-semibold">Son Eklenen Yazılar</h3><div className="mt-2 divide-y divide-border">{posts.slice(0, 4).map((post) => <div key={post.id} className="py-3"><p className="truncate text-sm font-medium">{post.title}</p><p className="mt-1 text-xs text-muted-foreground">{post.locale.toUpperCase()} · {post.status === 'published' ? 'Yayında' : 'Taslak'}</p></div>)}{posts.length === 0 && <p className="py-4 text-sm text-muted-foreground">Henüz blog yazısı yok.</p>}</div></section></div></div>
}

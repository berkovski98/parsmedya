...
import { FileText, LayoutDashboard, LogOut, Menu, PlusCircle, X } from 'lucide-react'
...
const links = [
  ...
  { href: '/admin/blog/new', label: 'Yeni Yazı', icon: PlusCircle },
]

export function AdminShell({ children }: { children: React.ReactNode }) {
  const [open, setOpen] = useState(false)
  const pathname = usePathname()
  const sidebar = <><div className="border-b border-primary-foreground/15 p-6"><p className="font-display text-xl font-bold">Pars<span className="text-accent">Medya</span></p><p className="mt-1 text-xs text-primary-foreground/65">Yönetim Paneli</p></div><nav className="flex-1 space-y-1 p-4">{links.map(({ href, label, icon: Icon }) => <Link key={href} href={href} onClick={() => setOpen(false)} className={cn('flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors', pathname === href ? 'bg-primary-foreground/15 text-primary-foreground' : 'text-primary-foreground/70 hover:bg-primary-foreground/10 hover:text-primary-foreground')}><Icon className="h-4 w-4" />{label}</Link>)}</nav><form action={logout} className="border-t border-primary-foreground/15 p-4"><button className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-primary-foreground/70 hover:bg-primary-foreground/10 hover:text-primary-foreground"><LogOut className="h-4 w-4" />Çıkış</button></form></>

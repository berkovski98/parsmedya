...
export async function AdminBlogList({ locale, searchParams }: { locale: Locale; searchParams: Promise<SearchParams> }) {
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    {!postsResult.error && <div className="mt-8 overflow-hidden rounded-xl border border-border bg-card"><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-secondary/60"><tr>{['Başlık', 'Kategori', 'Durum', 'Yayın tarihi', 'Güncellenme', 'İşlemler'].map((heading) => <th key={heading} className="px-4 py-3 font-semibold">{heading}</th>)}</tr></thead><tbody className="divide-y divide-border">{posts.map((post) => {
      ...
      return <tr key={post.id}><td className="max-w-xs px-4 py-4 font-medium">{post.title}</td><td className="px-4 py-4 text-muted-foreground">{post.category}</td><td className="px-4 py-4"><span className="rounded-full bg-secondary px-2.5 py-1 text-xs">{post.status === 'published' ? 'Yayında' : 'Taslak'}</span></td><td className="px-4 py-4 text-muted-foreground">{formatBlogDate(post.published_at, locale)}</td><td className="px-4 py-4 text-muted-foreground">{formatBlogDate(post.updated_at, locale)}</td><td className="px-4 py-4"><div className="flex flex-wrap gap-3"><Link href={`/admin/blog/${post.id}/edit`} className="font-medium text-accent hover:underline">Düzenle</Link>{post.status === 'published' && <Link href={isTurkish ? `/blog/${post.slug}` : `/en/blog/${post.slug}`} target="_blank" className="font-medium text-primary hover:underline">Görüntüle</Link>}{linkedId ? <Link href={`/admin/blog/${linkedId}/edit`} className="font-medium text-muted-foreground hover:underline">Çeviriyi Düzenle</Link> : <Link href={`/admin/blog/new?translateFrom=${post.id}`} className="font-medium text-muted-foreground hover:underline">{isTurkish ? 'İngilizce Versiyon Oluştur' : 'Türkçe Versiyon Oluştur'}</Link>}<form action={deletePost}><input type="hidden" name="id" value={post.id} /><input type="hidden" name="locale" value={locale} /><input type="hidden" name="slug" value={post.slug} /><DeleteButton /></form></div></td></tr>

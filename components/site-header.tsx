...
import { getCorporateLinks, getServiceGroups } from '@/lib/navigation'

export function SiteHeader({ locale = 'tr' }: { locale?: Locale }) {
  const [open, setOpen] = useState(false)
  const [mobileSection, setMobileSection] = useState<'corporate' | 'services' | null>(null)
  ...
  const closeMobile = () => { setOpen(false); setMobileSection(null) }
  ...
  return <header className="sticky top-0 z-50 border-b border-border/70 bg-background/90 backdrop-blur-md">
    <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-3 px-4 sm:px-6">
      ...
      <nav className="relative hidden items-center gap-5 md:flex lg:gap-7" aria-label={english ? 'Main menu' : 'Ana menü'}>
        ...
        <div className="group relative"><button className={cn('flex items-center gap-1 py-5 text-sm font-medium transition-colors hover:text-foreground', corporateLinks.some((item) => isActive(item.href)) ? 'text-foreground' : 'text-muted-foreground')} aria-haspopup="true">{english ? 'Corporate' : 'Kurumsal'}<ChevronDown className="h-4 w-4 transition-transform group-hover:rotate-180 group-focus-within:rotate-180" /></button><div className="invisible absolute left-0 top-full w-56 translate-y-1 rounded-xl border border-border bg-background p-2 opacity-0 shadow-xl transition-all group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100">{corporateLinks.map((item) => <Link key={item.href} href={item.href} className="block rounded-lg px-3 py-2.5 text-sm text-muted-foreground hover:bg-muted hover:text-foreground">{item.label}</Link>)}</div></div>
        <div className="group static"><button className={cn('flex items-center gap-1 py-5 text-sm font-medium transition-colors hover:text-foreground', isActive(servicesHref) ? 'text-foreground' : 'text-muted-foreground')} aria-haspopup="true">{english ? 'Services' : 'Hizmetlerimiz'}<ChevronDown className="h-4 w-4 transition-transform group-hover:rotate-180 group-focus-within:rotate-180" /></button><div className="invisible fixed left-4 right-4 top-16 w-auto translate-y-1 rounded-xl border border-border bg-background p-5 opacity-0 shadow-xl transition-all group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100 lg:left-1/2 lg:right-auto lg:w-[900px] lg:-translate-x-1/2"><div className="grid gap-x-6 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">{serviceGroups.map((group) => <section key={group.label}><h2 className="mb-2 text-xs font-bold uppercase tracking-wider text-accent">{group.label}</h2><div className="space-y-0.5">{group.links.map((item) => <Link key={item.href} href={item.href} className="block rounded-md px-2 py-1.5 text-xs leading-snug text-muted-foreground hover:bg-muted hover:text-foreground">{item.label}</Link>)}</div></section>)}</div><Link href={servicesHref} className="mt-5 block border-t border-border pt-4 text-sm font-semibold text-accent hover:text-accent/80">{english ? 'View All Services' : 'Tüm Hizmetlerimizi İnceleyin'} →</Link></div></div>
      ...
      <div className="hidden items-center gap-3 md:flex"><Link href={alternatePath(pathname)} className="text-xs font-semibold text-muted-foreground hover:text-foreground" aria-label={english ? 'Türkçe sürüm' : 'English version'}>{english ? 'TR' : 'EN'}</Link><Link href={contactHref} className={buttonVariants({ size: 'lg' })}>{english ? 'Get a Quote' : 'Teklif Al'}</Link></div>
      <button type="button" onClick={() => setOpen((value) => !value)} className="inline-flex h-10 w-10 items-center justify-center rounded-md md:hidden" aria-label={open ? (english ? 'Close menu' : 'Menüyü kapat') : (english ? 'Open menu' : 'Menüyü aç')} aria-expanded={open}>{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
    </div>
    {open && <div className="max-h-[calc(100vh-4rem)] overflow-y-auto border-t border-border bg-background md:hidden"><nav className="mx-auto flex max-w-6xl flex-col gap-1 px-4 py-4" aria-label={english ? 'Mobile menu' : 'Mobil menü'}>
      ...
      <Link href={contactHref} onClick={closeMobile} className={buttonVariants({ size: 'lg', className: 'mt-2' })}>{english ? 'Get a Quote' : 'Teklif Al'}</Link><Link href={alternatePath(pathname)} onClick={closeMobile} className="mt-2 rounded-md border border-border px-3 py-2.5 text-center text-sm font-semibold">{english ? 'TR — Türkçe' : 'EN — English'}</Link>
    </nav></div>}
  </header>
}
